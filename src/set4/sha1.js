...
class Sha1 {
     ...
     * @param   {number[]} [options.state] - previous 'state'
     * @param   {number} [options.desiredLength] - desiredLength
    ...
    static hash(msg, options) {
        ...
        if (options.state) {
            H[0] = options.state[0], H[1] = options.state[1], H[2] = options.state[2], H[3] = options.state[3], H[4] = options.state[4];
        }
        ...
        const msgLength = options.desiredLength
            ? options.desiredLength
            : (msg.length - 1) * 8;
        M[N - 1][14] = msgLength / Math.pow(2, 32);
        M[N - 1][14] = Math.floor(M[N - 1][14]);
        M[N - 1][15] = msgLength & 0xffffffff;
