 ...
 */
export function padMessageMD(msg: Buffer, blockSizeBits: number, msgSizeEndian: 'BE' | 'LE'): Buffer {
...
export function restoreHashFunctionState(hash: Buffer, hashFnType: 'sha1' | 'md4'): number[] {
    const state = [];
    if (hashFnType === 'sha1') {
 ...
 */
export function prepareTamperedInput(keyPlaceholder: Buffer, msg: Buffer, additionalInput: Buffer, msgSizeEndian: 'BE' | 'LE'): Buffer {
