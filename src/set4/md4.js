...
function core_md4(x, len, tamperedLen, state)
{
    /* append padding */
    x[len >> 5] |= 0x80 << (len % 32);
    x[(((len + 64) >>> 9) << 4) + 14] = tamperedLen ? tamperedLen : len;

    var a = state ? state[0] : 1732584193;
    var b = state ? state[1] : -271733879;
    var c = state ? state[2] : -1732584194;
    var d = state ? state[3] : 271733878;
 ...
 */
function hex2binl(hexstring) {
    var bin = Array();
    for (var i = 0; i < hexstring.length * 4 ; i += 8)
        bin[i>>5] |= (Number.parseInt(hexstring.slice(i/4, (i/4)+2), 16) & 0xFF) << (i % 32);
    return bin;
}
