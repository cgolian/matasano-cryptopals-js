...
export function initCbcMacClientServerProtocol(key: Buffer): {
    ...
    generateClientCreateTx: (from: number) => (txList: { to: number; amount: number }[]) => CbcMacRequest;
} {
    ...
    function generateClientCreateTx(from: number): (txList: { to: number; amount: number }[]) => CbcMacRequest {
        return function clientCreateTx(txList: { to: number; amount: number }[]): CbcMacRequest {
